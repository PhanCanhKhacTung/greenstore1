function themvaogiohang(x){
    ...
    var hinhsp = boxsp[0].children[0].src;
    var giasp = boxsp[1].children[0].innerText;
    var tensp = boxsp[2].innerText;
    var soluongsp = parseInt(boxsp[3].value);
    var sp = new Array(hinhsp, tensp, giasp, soluongsp);
...
function laydon(){
    ...
    for (let i = 0; i < giohang.length; i++) {
        ...
        <td><input type="number" min ="0" max = "10" value = "${giohang[i][3]}" 
                                        onchange = "tinhlaihoadon(this);" ></td>
        <td>${tt}</td>
        </tr>
        `
    }
...
function tinhalaidon(){
    ...
    if(sl==0){
        dongy = confirm("San pham se bi xoa khoi gio hang. OK?");
